...
class ImageDataLoader():
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False):
        ...
        self.data_files = [filename for filename in sorted(os.listdir(data_path)) \
                           if os.path.isfile(os.path.join(data_path,filename))]
        self.data_files.sort(key=lambda x: int(x.split('.')[0][4:]))
        ...
        if self.pre_load:
            ...
            for fname in self.data_files:
                ...
                #den = pd.read_csv(os.path.join(self.gt_path,os.path.splitext(fname)[0] + '.csv'), sep=',',header=None).iloc[:,:].values                        
